...
describe('P1-C menu UX contracts', () => {
  ...
  it('Lunch and WeeklyOffer keep honest empty states with booking CTAs', () => {
    ...
    assert.match(lunch, /weekdayLunchHours/)
    assert.match(lunch, /Mån–Fre/)
    assert.doesNotMatch(lunch, /Källa:/)
    assert.doesNotMatch(lunch, /Senast hämtad/)
    assert.doesNotMatch(lunch, /SourceAttribution/)
    assert.doesNotMatch(lunch, /Ingen lunch publicerad för idag i källan/)
